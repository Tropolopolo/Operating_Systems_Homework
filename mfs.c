...
int getInput(char* line, StrList* list)
{
	...
	list->clearList(list->entry, list->maxStrings);			//this is causeing issues
	int i = 0;
	for (; i < n; i++)
	{
		//printf("p_cmd[%d]: %s\n",i, p_cmd[i]);
...
void interpretTime(uint16_t time)
{
	uint16_t secmask = 31;
	uint16_t minmask = 480;
	uint16_t hrmask = 65024;
	printf("%d:%d:%d\n", time & hrmask, time & minmask, time & secmask);
...
int main()
{
	...
	do
	{
		...
		if (strncmp(list.head, "\n", strlen(list.head)) == 0)
			continue;
		fflush(stdin);
		//Open is finished, just needs further testing
		...
		//Close is finished, just needs further testing
		...
		if (strncmp(list.head, "get", strlen(list.head)) == 0)
		{
			...
			uint8_t val = 0;
			int flag = 0;
			fseek(fat.fatFile, file + dir[i].DIR_FileSize-1, SEEK_SET);
			fread(&val, 1, 1, fat.fatFile);
			//Easily broken if file of chars happens to not end with a char equal to 10
			if (val == 10)
				flag = 1;
		...
		//READ is Somewhat finished, still needs some adjusting
